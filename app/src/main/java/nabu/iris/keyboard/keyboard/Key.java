...
import nabu.iris.keyboard.keyboard.internal.KeySpecParser;
import nabu.iris.keyboard.keyboard.internal.KeyStyle;
import nabu.iris.keyboard.keyboard.internal.KeyVisualAttributes;
import nabu.iris.keyboard.keyboard.internal.KeyboardIconsSet;
import nabu.iris.keyboard.keyboard.internal.KeyboardParams;
import nabu.iris.keyboard.keyboard.internal.KeyboardRow;
import nabu.iris.keyboard.keyboard.internal.MoreKeySpec;
import nabu.iris.keyboard.latin.common.Constants;
import nabu.iris.keyboard.latin.common.StringUtils;

import static nabu.iris.keyboard.latin.common.Constants.CODE_OUTPUT_TEXT;
import static nabu.iris.keyboard.latin.common.Constants.CODE_SHIFT;
import static nabu.iris.keyboard.latin.common.Constants.CODE_SWITCH_ALPHA_SYMBOL;
import static nabu.iris.keyboard.latin.common.Constants.CODE_UNSPECIFIED;
...
public class Key implements Comparable<Key> {
    ...
    // Spacebar resize constructor
    public Key(final Key key, final int x, final int width) {
        mCode = key.mCode;
        mLabel = key.mLabel;
        ...
        mWidth = width;
        mHeight = key.mHeight;
        mDefinedWidth = key.mDefinedWidth * ((float)width / key.mWidth);
        mDefinedHeight = key.mDefinedHeight;
        mHitbox.set(x, key.mHitbox.top, x + width, key.mHitbox.bottom);
    ...
    // Dynamic emoji key constructor
    public Key(final String label, final int iconId, final int code, final String outputText,
               final MoreKeySpec[] moreKeys, final int x, final int y, final int width, final int height,
               final int backgroundType, final int actionFlags, final int labelFlags) {
        mX = x;
        mY = y;
        mWidth = width;
        mHeight = height;
        mDefinedWidth = width;
        mDefinedHeight = height;
        mHitbox.set(x, y, x + width, y + height);
        mHintLabel = null;
        mLabelFlags = labelFlags;
        mBackgroundType = backgroundType;
        mActionFlags = actionFlags;
        mMoreKeys = moreKeys;
        mMoreKeysColumnAndFlags = MORE_KEYS_FLAGS_HAS_LABELS;
        mLabel = label;
        mOptionalAttributes = OptionalAttributes.newInstance(outputText, CODE_UNSPECIFIED);
        mCode = code;
        mIconId = iconId;
